...
public class Player implements BattleshipsPlayer {
    ...
    private double[] myBoardInput;  //1 = hit, 0 = unknown, -1 = miss
    ...
    public void placeShips(Fleet fleet, Board board) {
        ...
        sizeX = board.sizeX();
        sizeY = board.sizeY();
        myBoardInput = new double[sizeX * sizeY];
    ...
    public Position getFireCoordinates(Fleet enemyShips) {
        brain.feed(myBoardInput);
        double[] outputs = brain.getOutputs();
        for (int i = 0; i < outputs.length; i++) {
           double output = outputs[i];
           if (output == 1.0) {
               System.out.println("Output[" + i + "] == 1");
               int x = (int) (output / myBoard.sizeX());
               int y = (int) (output - x);
               lastHit = new Position(x, y);
               if (getStatus(lastHit) != 0) {
                   System.out.println("Already hit here, recursion!");
                   return getFireCoordinates(enemyShips);
               }
               System.out.println("Firing at Position(" + x + ", " + y + ")");
               return lastHit;
           } 
        }
    ...
    public void hitFeedBack(boolean hit, Fleet enemyShips) {
        System.out.println("Hit Feedback!");
        if (hit) {
            registerHit(lastHit, 1);
        } else {
            registerHit(lastHit, -1);
        }
        totalHits++;
        if (enemyShips.getNumberOfShips() == 0) {
            //We killed all enemy ships
        }
        //Do nothing
    }    

    public void registerHit(Position p, double d) {
        int x = p.x * 10;
        int y = p.y;
        myBoardInput[x + y] = d;
    }
    
    public double getStatus(Position p) {
        int x = p.x * 10;
        int y = p.y;
        return myBoardInput[x + y];
    }
